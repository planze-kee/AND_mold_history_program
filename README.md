...
> **버전**: 2.1 &nbsp;|&nbsp; **최종 수정**: 2026-04-15 &nbsp;|&nbsp; **브랜치**: main
...
- XLSX 변경 시 기존 Word 파일 동기화 (파일명 rename 포함)
...
- **HWP 파싱 버그 수정**:
  - `管理番号` 값 누락 (Pattern 2 핸들러 추가)
  - `GATE型式/使用機械/契約日`가 `承認日` 값을 가져오는 누수 버그
- **이미지 탐색 버그 수정**:
  - 이미지 파일명 trailing underscore 제거 (`品名_` → `品名`)
  - `apply_to_word()` 재생성 시 이미지 누락 (4단계 fallback 추가)
