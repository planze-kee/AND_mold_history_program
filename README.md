...
> **버전**: 2.4 &nbsp;|&nbsp; **최종 수정**: 2026-04-29 &nbsp;|&nbsp; **브랜치**: main
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| 2.4 | 2026-04-29 | Excel 붙여넣기 후 이미지 탐색 실패 버그 수정 |
| 2.3 | 2026-04-17~21 | HWP 변환 버그 수정 2건 + PDF 로그 수정 + PDF 병합 개별 파일 미저장 옵션 |
...
- **Excel 붙여넣기 후 이미지 탐색 실패**: `ImageCache._normalize()` 추가
  - 모든 유니코드 공백(`\xa0`, 전각공백 등)·개행 정규화 (`"".join(stem.split())`)
  - 실수로 붙여넣어진 확장자 자동 제거 (예: `파일명.png` → `파일명`)
  - `find()` 진입 시 자동 적용, `金型写真` 컬럼 읽기 시에도 동일 적용
