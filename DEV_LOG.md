...
## v2.2 — EXE 용량 감축 + 동기화 증분 재생성 (2026-04-16)

### EXE 용량 감축 (`금형이력카드프로그램.spec`)

| 항목 | 변경 전 | 변경 후 | 효과 |
|------|---------|---------|------|
| `datas` | `data` + `YES` + `img` 번들 (~1.2GB) | `data`만 번들 | **~1.14GB 절감** |
| 빌드 방식 | `--onefile` (EXE 1개) | `--onedir` (`COLLECT` 구조) | 첫 실행 속도 ↑, 압축 오버헤드 제거 |
| `excludes` | 없음 | Qt 미사용 서브모듈 14개 + numpy/scipy 등 | 10~30MB 추가 절감 |
| `upx_exclude` | 없음 | `Qt5WebEngineCore.dll` 추가 | UPX 충돌 방지 |

- 배포 구조 변경: 단일 EXE → `dist/금형이력카드프로그램/` 폴더 전체 ZIP 배포
- `img/` 빈 폴더는 배포 ZIP에 포함 (런타임 데이터이므로 EXE에서 분리)
- `YES/`는 배포본에서 완전 제거 (개발/테스트용 데이터)

### 동기화 증분 재생성 (`src/core.py` — `DocxSyncManager`)

| 항목 | 내용 |
|------|------|
| `compute_row_hash()` 추가 | 행 내용 SHA1 해시 — 변경 감지 기준 |
| `compute_image_sig()` 추가 | 이미지 경로+mtime+size 서명 — 이미지 교체 감지 |
| `_resolve_image()` 헬퍼 추출 | sync 루프 내 이미지 탐색 로직을 독립 메서드로 분리 |
| `sync()` 증분 로직 | 해시+이미지 서명 동일 + DOCX 존재 → **스킵** (재생성 없음) |
| `force_all` 파라미터 | 강제 전체 재생성 플래그 |
| 템플릿 자동 감지 | `manifest._meta.template_hash` 저장, 변경 시 자동 `force_all=True` |
| manifest 스키마 확장 | `content_hash`, `image_sig`, `_meta.template_hash` 필드 추가 |
| 완료 메시지 개선 | `✓ 동기화 완료 — 신규 N / 갱신 N / 스킵 N` |

| 시나리오 | 변경 전 | 변경 후 |
|----------|---------|---------|
| 100건 중 1건 수정 | 100건 재생성 | 1건 재생성 + 99건 스킵 |
| 변경 없이 재실행 | 100건 재생성 | 전부 스킵 |
| 템플릿 교체 | 100건 재생성 | 자동 감지 후 100건 재생성 |

### GUI 변경 (`main.py`)

- 동기화 버튼 아래 **"강제 전체 재생성"** 체크박스 추가 (`sync_force_chk`)
- `run_sync()`에서 체크박스 값 → `force_all=` 파라미터로 전달
